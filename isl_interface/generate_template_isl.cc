...
static const std::unordered_map<std::string, std::vector<Signature<Kind>>>
    signatures{
        ...
        {"intersect", {modify_params_binary(), modify_set_binary()}},
...
static const std::
    map<std::pair<std::string, std::string>, std::vector<Signature<Kind>>>
        specificSignatures{
            {{"aff", "mod"}, {modify_set_unary()}},
            {{"pw_aff", "mod"}, {modify_set_unary()}},
...
static void printExtraTemplate(
  ...
  const auto& match = isStatic ? signature.returnType : signature.argTypes[0];
  auto specializedMatch = specialize(match, subs);
  for (size_t i = 0; i < specializedMatch.size(); ++i) {
    if (specializedMatch[i] != classKind[i]) {
      std::stringstream ss;
      ss << "= std::enable_if<std::is_same<";
      print(ss, specializedMatch[i]);
      ss << ", ";
      print(ss, classKind[i]);
      ss << ">::value>";
      extra.emplace_back(ss.str());
    }
  }
  if (extra.size() != 0) {
    printTemplate(extra);
...
static void printMethod(
    ...
    const Subs& subs,
    bool isStatic = false) {
  printExtraTemplate(classKind, signature, subs, isStatic);
  if (isStatic) {
    std::cout << "static ";
  }
  std::cout << "inline ";
  ...
  for (size_t i = 0; i < method.signature.argTypes.size(); ++i) {
    ...
    if (classes.count(type) == 0) {
      ...
      printType(type, signature.argTypes[j++], subs);
    }
    std::cout << "& arg" << i;
  }
  std::cout << ")";
...
static void printDefinition(
  ...
  if (exported.count(base) != 0) {
    for (auto method : exported.at(base)) {
      if (signatures.count(method.name) != 0) {
        for (const auto& signature : signatures.at(method.name)) {
          if (matches(classKind, signature, method) &&
